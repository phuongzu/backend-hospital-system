...
export interface IDoctor extends Document {
  ...
  avatar?: string; // New field for doctor image URL
  createdAt: Date;
  updatedAt: Date;
  // Virtuals
...
const doctorSchema = new Schema<IDoctor, IDoctorModel>(
  {
    ...
    languages: [{ 
      type: String, 
      trim: true,
      enum: ['English', 'Spanish', 'French', 'German', 'Chinese', 'Japanese', 'Korean', 'Arabic', 'Hindi', 'Other']
    }],
    education: [{ 
      type: String, 
      trim: true 
...
doctorSchema.index({ 'available_hours.monday.isAvailable': 1 });
...
// Pre-save middleware to validate available hours
doctorSchema.pre('save', function(next) {
  // Validate time format (HH:MM)
  const timeRegex = /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/;
  
  Object.values(this.available_hours).forEach(day => {
...
// Static method to find doctors by experience range
doctorSchema.statics.findByExperience = function(minYears: number, maxYears?: number) {
  const query: any = { years_of_experience: { $gte: minYears } };
  if (maxYears) {
    query.years_of_experience.$lte = maxYears;
  }
...
// Instance method to check if doctor is available on specific day and time
doctorSchema.methods.isAvailableAt = function(day: string, time: string): boolean {
  const daySchedule = this.available_hours[day as keyof typeof this.available_hours];
